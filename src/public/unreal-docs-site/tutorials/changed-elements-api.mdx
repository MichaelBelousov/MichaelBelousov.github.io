...
<Introduction
  skillLevel="Basic"
  durationInMinutes="45"
  summaryImageSrc="/unreal-docs-site/images/changed-elements-header.png">
...
The API requires the `changedelements:modify` scope to be added to your App. It can be added to your apps in the [Developer Portal](/my-apps/).
...
An iModel evolves over time. Every time a design file is changed and the iModel is synchronized, new [Changesets](https://www.itwinjs.org/reference/imodelhub-client/imodelhubclient/changeset/?term=changeset) are published to the iModel, updating the iModel data.
...
Here is an example request that shows how to enable the APIs change tracking for an iModel, you would just need to replace the project Id, iModel Id and authorization token with your own.
...
The next operation allows us to query the API to get the elements that have changed after processing is ready. To use this operation, you must provide the start and end changeset Ids that you want to obtain results for. To get changeset Ids, you can use [Get iModel Changesets API](/apis/imodels/operations/get-imodel-changesets/).

The API requires the `changedelements:read` scope to be added to your App. It can be added to your apps in the [Developer Portal](/my-apps/)
...
<TutorialSection
  ...
  actions={[
    ...
    <CodeBlock language="http"
    code={`GET https://api.bentley.com/changedelements/comparison?projectId=myProjectId&iModelId=myIModelId&startChangesetId=myStartChangesetId&endChangesetId=myEndChangesetId HTTP/1.1
Authorization: Bearer JWT_TOKEN
Content-Type: application/json`}/>,
...
Here is an example request that shows how to get the changed elements between two changesets, ensure to replace the project Id, iModel Id, start changeset Id, end changeset Id and authorization token with your own.
...
<Alert type="informational">
  The results are inclusive to both start and end changesets. This means
  that the changes found in both changesets will be contained in the results.
  This is important to think about because if you have an iModel with 2 Named
  Versions, A and B, and you want to get what has changed between A and B, you
  should *not* include A's changeset in the query. Consider the following example:
...
Here's the <a style={{textDecoration: "underline"}} href="/apis/changed-elements/operations/get-comparison/">documentation</a> for this operation.
...
<TutorialSection
  ...
  actions={[
    ...
    <CodeBlock language="typescript"
      code={`import { Version } from "@bentley/imodelhub-client";
...
import "./ChangedElementsWidget.scss";`}/>,
    <Title>Write a react hook for getting named versions</Title>,
    <CodeBlock language={"typescript"}
      code={`function useNamedVersions(props: { iModel: IModelConnection | undefined }) {
...
<TutorialSection
  ...
  actions={[
    ...
    <CodeBlock language={"typescript"}
      code={`import { ChangedElements } from "@bentley/imodeljs-common";
...
The `getComparison` function will use the [Get Comparison](/apis/changed-elements/operations/get-comparison/) endpoint to get the [ChangedElements](https://www.itwinjs.org/reference/imodeljs-common/entities/changedelements/) that were found between the given changesets, for a given iModel.
...
If authorization has been properly setup, you should obtain results from the API. To display the results in the viewport, let's emphasize the elements that have changed on the screen. We can use the [EmphasizeElements](https://www.itwinjs.org/reference/imodeljs-frontend/rendering/emphasizeelements/) class, which is a [FeatureOverrideProvider](https://www.itwinjs.org/reference/imodeljs-frontend/views/featureoverrideprovider/) that will highlight the elements on the viewport.

The `changedElements.elements` is an array that contains the element Ids of all elements that have changed in the iModel between the changesets that we are looking at. Here's more information about the [ChangedElements](https://www.itwinjs.org/reference/imodeljs-common/entities/changedelements/) result.
...
<TutorialSection
  ...
  actions={[
    ...
    <CodeBlock language={"typescript"}
      code={`import { DbOpcode, Id64String } from "@bentley/bentleyjs-core";
...
By now, you should have changed elements being emphasized in your view, but it would be better if we colorize them by their operation codes, see [DbOpcode](https://www.itwinjs.org/reference/bentleyjs-core/besqlite/dbopcode/).

To be able to colorize the elements, we will implement our own [FeatureOverrideProvider](https://www.itwinjs.org/reference/imodeljs-frontend/views/featureoverrideprovider). Create a new file `ChangedElementsFeatureOverrides.ts` and follow the code snippet to the right.

The provider's constructor finds which elements are inserted or updated based on their [DbOpcode](https://www.itwinjs.org/reference/bentleyjs-core/besqlite/dbopcode/). Then, it implements the `addFeatureOverrides` function that will colorize inserted elements as green, updated elements as blue and make everything else transparent gray.

<Alert type="informational">
  Displaying deleted elements is not as straightforward because the deleted
  elements are not present in the current iModel that we are displaying in the
  viewport. This can be done by implementing a{' '}
  <a
    style={{ textDecoration: 'underline' }}
  ...
  , but this is out of the scope for this tutorial.
</Alert>
...
To properly inspect an elements properties, including properties that do not exist on the element, we need to use [iTwin.js Presentation Library](https://www.itwinjs.org/learning/presentation/), which is out of scope for this tutorial.
...
[ChangedElements](https://www.itwinjs.org/reference/imodeljs-common/entities/changedelements/) class is defined in the @bentley/imodeljs-common package.
...
Contains the operation codes that tells us if the element was inserted, updated or deleted in the change. See [DbOpcode](https://www.itwinjs.org/reference/bentleyjs-core/besqlite/dbopcode/?term=dbopcode)
...
<TileList>
  ...
  <ResourceTile
    ...
    description={<>The iTwin Viewer is a configurable iTwin.js viewer that offers basic tooling and widgets out-of-the-box and can be further extended through the use of <a href="https://github.com/imodeljs/extension-sample" target="_blank">iTwin.js extensions</a>. This package contains the Viewer as a React component and some additional Typescript APIs.</>}
    isExternal={true}
  />
  <ResourceTile
    title="iTwin Viewer Create React App Template"
    href="https://www.npmjs.com/package/@itwin/cra-template-web-viewer"
    description={<>This is a template for applications that are based on the <a href="https://github.com/iTwin/viewer/tree/master/packages/modules/web-viewer-react" target="_blank">iTwin Viewer</a> for <a href="https://github.com/facebook/create-react-app" target="_blank">Create React App</a>.</>}
    isExternal={true}
  />
  <ResourceTile
