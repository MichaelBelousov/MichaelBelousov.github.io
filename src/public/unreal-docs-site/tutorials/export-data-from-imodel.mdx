...
<TutorialSection actions={[<CreateSampleAppButton scopes={['export:modify', 'export:read']} clientName="Export data from an iModel tutorial"/>]}>
...
> Make sure that your application is associated with **Export API** and has `export:modify export:read` scopes enabled.
...
<TutorialSection actions={[
  ...
  <CodeBlock language='http' code={`POST https://api.bentley.com/export/startExport HTTP/1.1`} />,
...
To start the Export, call the endpoint [Start Export](https://api.bentley.com/export/startExport)
...
To execute the request, go to [StartExport](/apis/export/operations/startExport/) page and use the "Try it out" section.
