...
<Introduction 
  skillLevel="Basic" 
  durationInMinutes="30" 
  summaryImageSrc="/unreal-docs-site/images/itwin-viewer-hello-world-sample.gif" >
...
<TutorialSection hasTitle={false} actions={[
  <Title>Toggle import</Title>,
  <CodeBlock language='typescript' code={`import { Toggle } from '@bentley/ui-core';`} />
]}>
...
<CodeBlock language='typescript' code={`if (location === StagePanelLocation.Right) {
...
<TutorialSection actions={[
  <Title>Passing the onChange prop to the Toggle component</Title>,
...
To do this, we need to pass the `onChange` prop to the `Toggle` component.
...
<CodeBlock language='typescript' code={`import { ColorDef } from '@bentley/imodeljs-common';
