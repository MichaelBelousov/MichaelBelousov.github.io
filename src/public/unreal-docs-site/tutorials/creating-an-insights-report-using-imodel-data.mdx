...
<TutorialSection actions={[<CreateSampleAppButton scopes={['openid','insights:read','insights:modify']} clientName="Creating an insights report using imodel data tutorial"/>]}>
...
> Make sure that your application is associated with **Visualization** and **Insights** APIs and has `openid insights:read insights:modify` scopes enabled.
...
> | scope:     | Must have `openid` scope for authorization and `insights:read` & `insights:modify` scopes for Reporting platform APIs.  |
