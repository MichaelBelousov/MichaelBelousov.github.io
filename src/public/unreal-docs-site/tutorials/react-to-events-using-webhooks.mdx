...
In this tutorial, you will learn how to use the Webhooks REST API to create and manage your webhooks. Also you will learn how to validate the webhook signature and process received event messages in your application.
...
- Already registered your own application/client on iTwin Platform.
  ...
  - Make sure that your application/client is associated with **Webhooks API** and has `webhooks:read` and `webhooks:modify` scopes enabled.
- Any existing **iModel** that your application/client has access and contains at least one changeset that has not a named version created on.
  - In order to create one you need to run the synchronization for an iModel which is described in this [tutorial](/tutorials/synchronization-tutorial). Or you can [create a new iModel](/tutorials/create-test-imodel-sample) from Bentley provided sample.
...
<Alert type="informational">
  If you are running into "tsc is not recognized" problems, try installing
  typescript globally "npm install typescript -g".
</Alert>
...
1. Deploy by pushing the code <https://devcenter.heroku.com/articles/git#deploying-code>.
...
<Alert type="informational">
  Don't forget to replace the "IMODEL_ID" placeholder value with an existing
  iModel ID and "HOSTNAME" placeholder value with your deployed application
  hostname.
</Alert>
...
<Alert type="informational">
  Note that the webhook secret should not be shared with anyone and treated as a
  private key.
</Alert>
...
<TileList>
  <ResourceTile
    title="Events API group documentation"
    href="/api-groups/events"
    description="An overview and detailed Events API group documentation."
  />
  <ResourceTile
