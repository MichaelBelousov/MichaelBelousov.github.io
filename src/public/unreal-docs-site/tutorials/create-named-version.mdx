...
- Synchronization of design files to the iModel configured and synchronized.
  - Instructions to synchronize a file can be found [here](/tutorials/synchronization-tutorial)

</TutorialSection>

<TutorialSection actions={[<CreateSampleAppButton scopes={['realitydata:read','imodels:read','imodels:modify','library:read','storage:read','storage:modify']} clientName="Create iModel Named Version tutorial"/>]}>
...
> Make sure that your application is associated with **iModels**, **Library**, **RealityData** and **Storage** APIs and has `realitydata:read imodels:read imodels:modify library:read storage:read storage:modify` scopes enabled.
...
1. Go to [Get Project iModels - iModels](/apis/imodels/operations/get-project-imodels/)
...
<TutorialSection actions={[
  ...
  <CodeBlock language="http" code={`GET https://api.bentley.com/imodels/?projectId=PROJECT_ID HTTP/1.1`} />,
...
<Alert type="informational">
  Note: please replace dynamic parameters (JWT_TOKEN and PROJECT_ID) with
  relevant values from previous steps.
</Alert>

You can execute the request in [Get project iModels documentation](/apis/imodels/operations/get-project-imodels/) page, "Try it" section.
...
You can execute the request in [Get iModel Changesets documentation](/apis/imodels/operations/get-imodel-changesets/) page, "Try it" section.
...
You can execute the request in [Create iModel Named Version documentation](/apis/imodels/operations/create-imodel-named-version/) page, "Try it" section.
